...
const userValidator = async (req,res,next)=>{
  ...
  if(!user.isValid(fullName)){
    return res.status(400).send({status:false,message:"fullName needs good formate"})
  }
  
if(!user.isValid(userName)){
  return res.status(400).send({status:false,message:"userName needs good formate"})
}

  const userNameCheck = await users.find({userName:userName}).toArray()
  if(userNameCheck.length>0){
    return res.status(400).send({status:false,message:"userName already existed "})
  }
  ...
  if (!/^(?:(?:\+|0{0,2})91(\s*[\-]\s*)?|[0]?)?[6789]\d{9}$/.test(mobileNumber)) {
    ...
    const mobileNumberCheck = await users.find({mobileNumber:mobileNumber}).toArray()
    if(mobileNumberCheck.length>0){
      return res.status(400).send({status:false,message:"mobileNumber already existed user another number"})
    }
